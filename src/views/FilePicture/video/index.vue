<template>
  <div>
    <el-card class="card-layout">
      <el-button type="primary" size="small" v-if="hasPerm(`video:down`)" class="el-icon-plus" style="float: right"
                 round @click="downsImage">
        上传视频
      </el-button>
      <el-button type="primary" @click="log" v-if=" hasPerm(`video:log`)" size="small" round class="el-icon-date"
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="props">
              ...
              <svg-icon icon-class="video" v-if='props.row.type==="2"'/>
            </template>
          </el-table-column>
          <el-table-column
            prop="folderName"
          ...
          <el-table-column
            ...
            <template slot-scope="props">
              ...
              <el-button type="danger" size="small" v-if="props.row.type===`1` && hasPerm(`video:dele`)" round
                         @click="deleInformation(props.row.id)">删除
              </el-button>
              <el-button type="danger" size="small" v-if="props.row.type===`2` && hasPerm(`video:delet`)" round
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="props">
              ...
              <el-button type="danger" size="small" round v-if="hasPerm(`video:delet`)"
                         @click="deleFile01(props.row.id)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        <div v-if="hasPerm(`video:delet`)">
      ...
      <el-drawer
        ...
        <div style="padding-left: 10px;padding-right: 10px;height: 80%;overflow-x: hidden">
          ...
          <el-form ref="form" label-position="left" :model="img" label-width="80px">
            ...
            <el-form-item style="">
              <el-button type="text" @click="downs(img.url,img.name)">下载视频</el-button>
              <el-button type="text"><a target="_blank" :href="img.url">打开视频URL</a></el-button>
              <el-button type="text">
                 <span class="copy-btn" v-clipboard:copy="img.url" v-clipboard:success="onCopy">复制视频URL</span>
              </el-button>
            </el-form-item>
          </el-form>
        </div>
    ...
    methods: {
      ...
      onSubmit() {
        if(this.tag.name===''){
          ...
          addFolder(this.tag.name, 3).then(res => {
            if (res.queue.code === 1) {
      ...
      fetchData() {
        ...
        if (this.currentPage === 1) {
          ...
          findFileData(this.currentPage, '', 5, name, sort, '3').then(res => {
            if (res.queue.code === 1) {
      ...
      fetchDataLog() {
        let sort = 'desc'
        let name = 'created_time'
        findJournal(this.currentPageLog, this.sizeLog, name, sort, '3').then(res => {
          if (res.queue.code === 1) {
      ...
      downsImage() {
        if (this.flag === true) {
          ...
          this.location = '视频管理/' + this.locationUrl + '/'
        }
        this.visible = true
      },
      deleInformation(id) {
        ...
        if (search !== '' || search !== undefined) {
          ...
          if (this.flag === true) {
            if (this.currentPage === 1) {
              ...
              findFileData(this.currentPage, search, 5, name, sort, '3').then(res => {
                if (res.queue.code === 1) {
                  res.data.list.forEach(item => {
                    item.type = '2'
                    item.folderName = item.fileName
