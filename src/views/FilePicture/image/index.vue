<template>
  <div>
    <el-card class="card-layout">
      <el-button type="primary" size="small" v-if="hasPerm(`image:down`)" class="el-icon-plus" style="float: right"
                 round @click="downsImage">
        上传图片
      </el-button>
      <el-button type="primary" @click="log" v-if=" hasPerm(`image:log`)" size="small" round class="el-icon-date"
                 style="float: right">&nbsp;&nbsp;日志查询
      </el-button>
      <el-input style="width: 300px" type="text" maxlength="10" show-word-limit prefix-icon="el-icon-search" size="small" clearable v-model="search"
                placeholder="请输入搜索内容"/>
    </el-card>
    <el-card class="card-layout">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="props">
              ...
              <svg-icon icon-class="imageUpload" v-if='props.row.type==="2"'/>
            </template>
          </el-table-column>
          <el-table-column
            prop="folderName"
          ...
          <el-table-column
            ...
            <template slot-scope="props">
              ...
              <el-button type="danger" size="small" v-if="props.row.type===`1` && hasPerm(`image:dele`)" round
                         @click="deleInformation(props.row.id)">删除
              </el-button>
              <el-button type="danger" size="small" v-if="props.row.type===`2` && hasPerm(`image:delet`)" round
      ...
      <div v-show="flag===false">
        <el-table
          ...
          <el-table-column
            ...
            align="center"
            width="100px">
            <svg-icon icon-class="imageUpload"/>
          </el-table-column>
          <el-table-column
            prop="folderName"
            label="名称"
          ...
          <el-table-column
            ...
            <template slot-scope="props">
              ...
              <el-button type="danger" v-if="hasPerm(`image:delet`)" size="small" round
                         @click="deleFile01(props.row.id)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        <div v-if="hasPerm(`image:delet`)">
      ...
      <el-drawer
        ...
        <div style="padding-left: 10px;padding-right: 10px;height: 80%;overflow-x: hidden">
          <el-carousel indicator-position="outside">
            <el-carousel-item v-for="val in item" :key="val" :initial-index="false">
          ...
          <el-form ref="form" label-position="left" :model="img" label-width="80px">
            ...
            <el-form-item style="">
              <el-button type="text" @click="downs(img.url,img.name)">下载图片</el-button>
              <el-button type="text"><a target="_blank" :href="img.lookurl">打开图片URL</a></el-button>
              <el-button type="text">
                 <span class="copy-btn" v-clipboard:copy="img.url" v-clipboard:success="onCopy">复制图片URL</span>
              </el-button>
            </el-form-item>
          </el-form>
        </div>
    ...
    methods: {
      ...
      onSubmit() {
        if(this.tag.name===''){
          ...
          addFolder(this.tag.name, 2).then(res => {
            if (res.queue.code === 1) {
      ...
      fetchData() {
        ...
        if (this.currentPage === 1) {
          ...
          findFileData(this.currentPage, '', 5, name, sort, '2').then(res => {
            if (res.queue.code === 1) {
      ...
      fetchDataLog() {
        let sort = 'desc'
        let name = 'created_time'
        findJournal(this.currentPageLog, this.sizeLog, name, sort, '2').then(res => {
          if (res.queue.code === 1) {
      ...
      downsImage() {
        if (this.flag === true) {
          ...
          this.location = '图片管理/' + this.locationUrl + '/'
        }
        this.visible = true
      },
      deleInformation(id) {
        ...
        if (search !== '' || search !== undefined) {
          ...
          if (this.flag === true) {
            if (this.currentPage === 1) {
              ...
              findFileData(this.currentPage, search, 5, name, sort, '2').then(res => {
                if (res.queue.code === 1) {
                  res.data.list.forEach(item => {
                    item.type = '2'
                    item.folderName = item.fileName
