  ...
  <el-card class="card-layout">
    <el-form :model="doctor" ref="doctor" :rules="rules" label-width="100px" style="width: 40%;margin:auto">
      <el-form-item label="头像:" prop="url">
        <img v-if="doctor.url!==``" :src="doctor.url" style="border-radius: 50%;margin-left: 34%" width="80px" height="80px"/>
      ...
      <el-form-item label="医生岗位:" prop="job">
        <el-input v-model="doctor.job" size="small" type="text" maxlength="10" show-word-limit placeholder="请输入医生岗位"/>
      </el-form-item>
      <el-form-item label="擅长主治:" prop="adeptDisease">
        <el-input type="textarea" style="width: 100%" :row="4" maxlength="100" show-word-limit v-model="doctor.adeptDisease" placeholder="请输入医生的擅长主治"/>
      </el-form-item>
      <el-form-item label="个人简介:" prop="introduce">
        <el-input type="textarea" style="width: 100%" :row="4" maxlength="200" show-word-limit v-model="doctor.introduce" placeholder="请输入个人简介"/>
      </el-form-item>
      <el-form-item label="出诊医院:" prop="hospitalName">
        <el-input v-model="doctor.hospitalName" size="small" type="text" maxlength="20" show-word-limit placeholder="请输入出诊医院名"/>
      </el-form-item>
      <el-form-item label="医院地址:" prop="address">
        <el-input v-model="doctor.address" size="small" type="text" maxlength="20" show-word-limit placeholder="请输入医院地址"/>
      </el-form-item>
...
<script>
  ...
  import {addDoctors,findHealthThemeAll} from '@/api/health/doctors'
  export default {
    name: "index",
    ...
    methods: {
      onSubmit(doctor) {
        this.$refs[doctor].validate((valid) => {
          if (valid) {
            addDoctors(this.doctor).then(res => {
              if (res.queue.code === 1) {
